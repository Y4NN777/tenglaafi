...
try:
    from evaluation.scripts.metrics import RAGMetrics  # type: ignore
except Exception:
...
def _to_5(x: Optional[float]) -> Optional[float]:
    if x is None:
        return None
    return round(_clamp(float(x)) * 5.0, 2)


def _combine_pertinence_5(
    semantic_similarity: Optional[float],
    answer_completeness: Optional[float],
) -> Optional[float]:
    """
    Pertinence (/5) = 0.6 * semantic_similarity + 0.4 * answer_completeness (puis ×5)
    - si une seule métrique existe, on l'utilise seule
    - toujours borné entre 0 et 5
    """
    if semantic_similarity is None and answer_completeness is None:
        return None
    if semantic_similarity is None:
        base = _clamp(answer_completeness)
    elif answer_completeness is None:
        base = _clamp(semantic_similarity)
    else:
...
def run_evaluation(
    ...
    for q in questions:
        ...
        metrics["retrieval_precision_5"] = _to_5(rp)
        metrics["pertinence_5"] = _combine_pertinence_5(sim, comp)
        if qid in ratings:
...
if __name__ == "__main__":
    ...
    out_json = EVAL / "evaluation_results" / "evaluation_results.json"
    out_csv = EVAL / "evaluation_results" / "evaluation_results.csv"
    ...
    if summary.get("retrieval_precision_5_avg") is not None:
        print(f"Précision retrieval (/5) moy. : {summary['retrieval_precision_5_avg']}")
    ...
    if summary.get("pertinence_5_avg") is not None:
        print(f"Pertinence (/5) moy.     : {summary['pertinence_5_avg']}")
